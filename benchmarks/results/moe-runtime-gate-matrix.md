...
repair/re-gate (#11)**. Pending rows
...
| #18 | `f37be96` -> `883bda2` + repairs through `b5d2262` | Gate/up compute overlapped with down-projection reads | Lifecycle repair passed four RED regressions; cross-row prefix ordering RED `b42a0f7` and fix `b5d2262` prevent sibling prefix writes after the first prefix kernel starts. The focused files pass 108 tests and the full suite passes 2,061 / 4 skipped. | On macOS 26.5.2 the candidate completed a 1,280-token canary (6.2306 decode tok/s) and a natural 1,905-token run (5.3603 decode tok/s, 89,145,802,612 B peak, 1,768,689,893,376 read bytes, 318,836 reads) with exact token parity and zero reported failures. The next retained-base arm panicked in `IOGPUFamily`/`AGXG17X`, but another agent was reportedly experimenting; no second large MLX process was live in the stackshot, so prior overlap is possible but unproven. The arm is invalid for performance and causal comparison. | **Investigation hold: post-update candidate passes, confounded base panic, same-bank overlap remains** | **Not integrated; keep open and unattributed pending exclusive-GPU bounded base/candidate reruns** |
