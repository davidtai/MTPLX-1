...
| #18 | `f37be96` -> `883bda2` + repairs through `b5d2262` | Gate/up compute overlapped with down-projection reads | Lifecycle repair passed four RED regressions; cross-row prefix ordering RED `b42a0f7` and fix `b5d2262` prevent sibling prefix writes after the first prefix kernel starts. The focused files pass 108 tests and the full suite passes 2,061 / 4 skipped. | Base completed naturally at 6.5302 decode tok/s. Repaired exact-prefix lanes completed through 1,024 tokens (6.2355 decode tok/s, 88,894,111,604 B peak, 1,024,025,296,896 read bytes, zero reported failures), but the next 2,048 lane caused a second watchdog panic. The benchmark Metal queue was stuck 93.559 s in `IOGPUFamily`/`AGXG17X`; no memory pressure. Installed macOS 26.5.1 lacks Apple's 26.5.2 `IOGPUFamily` race fix. | **Investigation hold: GPU-driver panic plus same-bank overlap remain** | **Not integrated; no sustained rerun on 26.5.1, OS update and resource isolation required** |
