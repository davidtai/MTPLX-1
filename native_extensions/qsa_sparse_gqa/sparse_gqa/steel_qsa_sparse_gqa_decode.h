...
//     not cosmetic:
//
//     The prefill selector (``_select_eager``) SORTS its top-k ascending, so
//     there the valid entries really are a leading prefix and the prefix cut
//     is exact.  The DECODE selector (``QSAIndexer._select_m4``) does not
//     sort: it hands ``mx.argpartition``'s raw output straight to
//     ``qsa_m4_row_tokens``, whose predicate is ``block < visible_blocks``
//     evaluated on EVERY slot.  Applying the prefix cut to an unsorted row
...
mtplx_qsa_sparse_gqa_decode_split(
  ...
  // (kernels/qwen4_qsa_m4_indexer.py, _SRC_ROW_TOKENS).
