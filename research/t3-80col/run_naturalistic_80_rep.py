#!/usr/bin/env python3
"""Naturalistic short-prompt arm driver for the T3 **80-envelope column**
(2026-07-22), one rep per invocation. Sibling of
research/t3-88col-1024-nat/run_naturalistic_rep.py, retargeted at the
`hy3-oq2e-rq4-80` preset (islands 69, 10 streamed) instead of the
full-residency `-88e` preset -- otherwise identical mechanics.
...
filler token) is the SAME derivation as the 88e sibling -- direct-probed
there, reused as-is here since the tail content and chat-template plumbing
are identical; only the preset differs (islands 69 vs 79, i.e. this arm
streams 10 layers instead of being fully resident).

Usage (never invoked directly except by run_80col_inner.sh, which already
holds the flock + has qwen stopped):
    PYTHONPATH=<worktree> python3 \
        research/t3-80col/run_naturalistic_80_rep.py \
        --output-json evals/tier2/t3_80col_naturalistic_repN.json
"""
...
def main() -> int:
    ...
    print(f"[naturalistic-80] invoking bench.main({argv!r})", file=sys.stderr)
