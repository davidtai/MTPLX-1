#!/usr/bin/env python3
"""CPU-only, targeted-field validator + aggregator for the T3 80-envelope-
column real-prefill matrix receipts (2026-07-22): `hy3-oq2e-rq4-80`
(islands 69, 10 streamed), bf16 KV, K1 (+free AR), 3 reps each --

  ARM 1024:         contexts=1024,  output-tokens=256, no KV override.
  ARM 16384:        contexts=16384, output-tokens=256, max-live-kv=16640,
                     memory-limit override 97525956608 (90.8281 GiB).
  ARM 65536:        contexts=65536, output-tokens=256, max-live-kv=65792,
                     memory-limit override 113632083968 (105.8281 GiB).
  ARM naturalistic: contexts=265,   output-tokens=256, no KV override.
...
research/t3-kselect-88e16k-bf16/validate_kselect.py and
research/t3-88col-1024-nat/validate_88col.py.
...
Validates per row:
  ...
  3. The 6 mandatory metrics emitted: ingestion_tok_s, prompt_eval_time_s,
     ...
     is CORRECT), decode_expert_cache_hit_rate (this preset streams 10 of
     ~79 layers -- islands 69 -- so this is EXPECTED to be a real non-null
     number here, unlike the 88e full-residency preset where null is
     correct; reported and gated as "must be non-null" for K1 rows), and
...
Usage:
    PYTHONPATH=<worktree> python3 research/t3-80col/validate_80col.py
"""
...
ARMS = {
    "1024": {
        "prefix": "t3_80col_1024_rep",
        "expected_token_count": 1024,
        "limit_gib": 87.0000,
        "label": "ARM 80x1024 (real prefill)",
    },
    "16384": {
        "prefix": "t3_80col_16384_rep",
        "expected_token_count": 16384,
        "limit_gib": 97525956608 / GIB,
        "label": "ARM 80x16384 (real prefill)",
    },
    "65536": {
        "prefix": "t3_80col_65536_rep",
        "expected_token_count": 65536,
        "limit_gib": 113632083968 / GIB,
        "label": "ARM 80x65536 (real prefill)",
    },
    "naturalistic": {
        "prefix": "t3_80col_naturalistic_rep",
        "expected_token_count": 265,
        "limit_gib": 87.0000,
        "label": "ARM naturalistic-80 (~265 tokens)",
    },
}
...
def _validate_arm(arm_key: str, spec: dict[str, Any]) -> tuple[bool, dict[str, Any]]:
    ...
    for rep in REPS:
        ...
        for row in info["rows"]:
            ...
            # Islands 69 (10 of ~79 routed layers streamed): unlike the 88e
            # full-residency preset, decode_expert_cache_hit_rate is EXPECTED
            # to be a real non-null number at this preset -- gate on it.
