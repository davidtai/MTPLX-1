#!/usr/bin/env python3
"""CPU-only, targeted-field validator + aggregator for the T3 88-column
completion receipts (2026-07-22): hy3-oq2e-rq4-88e, bf16 KV, K1 (+free AR),
3 reps each --

  ARM 1: fresh matrix-shape cell, contexts=1024, output-tokens=256
         (evals/tier2/t3_88col_1024_rep{1,2,3}.json).
  ARM 2: naturalistic short-prompt arm, contexts=265, output-tokens=256
         (evals/tier2/t3_88col_naturalistic_rep{1,2,3}.json).
...
research/t3-kselect-88e16k-bf16/validate_kselect.py.

Validates per row:
  1. prompt_identity.token_count == expected (1024 for arm1, 265 for arm2).
  2. prompt_release_valid is True for BOTH arms (arm2 lands on the normal
     filler-branch metadata -- prompt_policy=="realistic_programming_v1",
     prompt_release_valid=True, prompt_tail_preserved=True -- see
     run_naturalistic_rep.py's header for why 265, not the naively-exact
     264, was required). Reported explicitly either way, never assumed.
  3. The 6 mandatory metrics emitted: ingestion_tok_s, prompt_eval_time_s,
     ...
     is CORRECT), decode_expert_cache_hit_rate (structurally None at this
     preset's full residency -- islands=79, 0 streamed -- CORRECT, not a
     failure), hard_peak_memory_bytes (model-level).
  4. kv_quant is None/off (bf16) via model["runtime_config"]["kv_quant"].
  5. hard_peak_memory_bytes <= 96 GiB (the preset's own declared limit;
     no override was used or needed for either arm).

Then prints a per-arm x per-lane (AR/K1) x per-rep table of the 6 metrics
and per-lane means.

Usage:
    PYTHONPATH=<worktree> python3 research/t3-88col-1024-nat/validate_88col.py
"""
...
GIB = 1024**3
LIMIT_GIB = 96.0  # preset's own declared limit; no override used
...
ARMS = {
    "arm1_1024": {
        "prefix": "t3_88col_1024_rep",
        "expected_token_count": 1024,
        "label": "ARM1 fresh 1024 cell",
    },
    "arm2_naturalistic": {
        "prefix": "t3_88col_naturalistic_rep",
        "expected_token_count": 265,
        "label": "ARM2 naturalistic ~265 cell",
    },
}
