#!/usr/bin/env python3
"""Naturalistic short-prompt arm driver for the T3 **48-envelope column**
(2026-07-22), one rep per invocation. Sibling of
research/t3-80col/run_naturalistic_80_rep.py, retargeted at the
`hy3-oq2e-rq4-48-cachelru` preset (ZERO islands, cache-heavy LRU -- David's
...
Usage (never invoked directly except by run_48col_inner.sh, which already
holds the flock + has qwen stopped):
    PYTHONPATH=<worktree> python3 \
        research/t3-48col/run_naturalistic_48_rep.py \
        --output-json evals/tier2/t3_48col_naturalistic_repN.json
"""
...
def main() -> int:
    ...
    print(f"[naturalistic-48] invoking bench.main({argv!r})", file=sys.stderr)
