#!/usr/bin/env bash
# INNER command for the T3 bf16-KV K-SELECTION window REP 3 recovery run
# (2026-07-22). Single rep only -- rep 1 already passed cleanly in the
# original window (see run_kselect_rep3.sh header for the root-cause
# writeup on why rep 3 needs a retry, not a full 3-rep re-run).
#
...
RC=$?
if [[ "$RC" != 0 ]]; then
  echo "[run_kselect_rep3_inner_bf16] mtplx resolution assertion FAILED -- aborting before any GPU work" >&2
  exit 9
fi
...
echo "[run_kselect_rep3_inner_bf16] === preset=$PRESET rep=$rep (RETRY) starting $(date -u +%FT%TZ) ===" | tee "$log" >&2
"$PY" scripts/benchmark_q2_mtp_depth_matrix.py \
  ...
  >> "$log" 2>&1
rc=$?
echo "[run_kselect_rep3_inner_bf16] === preset=$PRESET rep=$rep (RETRY) exit=$rc $(date -u +%FT%TZ) ===" >> "$log"
echo "[run_kselect_rep3_inner_bf16] preset=$PRESET rep=$rep (RETRY) exit=$rc $(date -u +%FT%TZ)" >&2
