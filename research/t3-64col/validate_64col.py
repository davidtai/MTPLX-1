#!/usr/bin/env python3
"""CPU-only, targeted-field validator + aggregator for the T3 64-envelope-
column real-prefill matrix receipts (2026-07-22): `hy3-oq2e-rq4-64-cachelru`
...
bf16 KV, K1 (+free AR), 3 reps each --
  ...
  ARM 16384:        contexts=16384, output-tokens=256, max-live-kv=16640,
                     ...
                     71 GiB limit ADMITs; cache shrinks to absorb KV growth).
  ARM 65536:        contexts=65536, output-tokens=256, max-live-kv=65792,
                     no memory-limit override needed (same reason).
...
Validates per row:
  ...
  3. The 6 mandatory metrics emitted: ingestion_tok_s, prompt_eval_time_s,
     ...
     routed layers through one shared LRU cache -- ZERO islands, even more
     completely streaming than the 80-column's islands-69 preset (10 of ~79
     streamed) -- so this is EXPECTED to be a real non-null number on EVERY
     row here; reported and gated as "must be non-null" for K1 AND AR rows),
     and hard_peak_memory_bytes (model-level).
...
Usage:
    PYTHONPATH=<worktree> python3 research/t3-64col/validate_64col.py
"""
...
ARMS = {
    "1024": {
        "prefix": "t3_64col_1024_rep",
        "expected_token_count": 1024,
        "limit_gib": 71.0000,
        "label": "ARM 64x1024 (real prefill)",
    },
    "16384": {
        "prefix": "t3_64col_16384_rep",
        "expected_token_count": 16384,
        "limit_gib": 71.0000,
        "label": "ARM 64x16384 (real prefill)",
    },
    "65536": {
        "prefix": "t3_64col_65536_rep",
        "expected_token_count": 65536,
        "limit_gib": 71.0000,
        "label": "ARM 64x65536 (real prefill)",
    },
    "naturalistic": {
        "prefix": "t3_64col_naturalistic_rep",
        "expected_token_count": 265,
        "limit_gib": 71.0000,
        "label": "ARM naturalistic-64 (~265 tokens)",
    },
}
