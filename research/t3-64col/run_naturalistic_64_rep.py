#!/usr/bin/env python3
"""Naturalistic short-prompt arm driver for the T3 **64-envelope column**
(2026-07-22), one rep per invocation. Sibling of
research/t3-80col/run_naturalistic_80_rep.py, retargeted at the
`hy3-oq2e-rq4-64-cachelru` preset (ZERO islands, cache-heavy LRU -- David's
...
Usage (never invoked directly except by run_64col_inner.sh, which already
holds the flock + has qwen stopped):
    PYTHONPATH=<worktree> python3 \
        research/t3-64col/run_naturalistic_64_rep.py \
        --output-json evals/tier2/t3_64col_naturalistic_repN.json
"""
...
def main() -> int:
    ...
    print(f"[naturalistic-64] invoking bench.main({argv!r})", file=sys.stderr)
