#!/usr/bin/env bash
# OUTER launcher: T3 64-envelope-column real-prefill matrix guarded window
# (2026-07-22). Matrix K locked = K1 (2e2ed99, #130 comment 5048661802).
#
# Preset `hy3-oq2e-rq4-64-cachelru` (ZERO islands, cache-heavy LRU -- David's
...
# invocations): 64x1024, 64x16k, 64x64k, naturalistic-64. All real long-code
...
#
# Memory-limit overrides (harness-derived by research/t3-64col/preflight.py,
# never hand-derived): NONE needed for this column -- every arm ADMITs at
# the preset's own declared 71 GiB limit. Its ~50 GiB elastic cache shrinks
# to absorb KV growth (49.99 -> 46.48 -> 31.25 GiB as kv_tokens goes
# 4096 -> 16640 -> 65792) while fixed_bytes grows in step (20.64 -> 24.47 ->
# 39.47 GiB), staying inside 71 GiB throughout (margins 0.3667 / 0.0536 /
# 0.2856 GiB). This differs from the 80-column (islands 69) preset, whose
# tiny ~2 GiB cache has no room to give and needs an explicit --memory-limit
# override at every context arm above 1024.
#
# Sign-off (David's rule 7): every derived limit here (71 GiB) sits well
# under the sub-85/90 GiB sign-off band and the 112 GiB wired knob -- no
# fresh ask needed for this column.
#
# CPU-only preflight (research/t3-64col/preflight.py) MUST pass (all
...
echo "[run_64col] wired-knob (recorded, not gated at this margin): sysctl iogpu.wired_limit_mb..." >&2
ACTUAL_WIRED_LIMIT_MB="$(sysctl -n iogpu.wired_limit_mb 2>/dev/null)"
echo "[run_64col] iogpu.wired_limit_mb=$ACTUAL_WIRED_LIMIT_MB" | tee "$WT/evals/tier2/t3_64col_wired_knob.txt" >&2

echo "[run_64col] CPU-only admission preflight (zero GPU touch) for all four arms..." >&2
PYTHONPATH="$WT" "$PY" research/t3-64col/preflight.py \
  --output "$WT/evals/tier2/t3_64col_admission_preflight.json" \
  2> "$WT/evals/tier2/t3_64col_admission_preflight.log"
PREFLIGHT_RC=$?
if [[ "$PREFLIGHT_RC" != 0 ]]; then
  echo "[run_64col] PREFLIGHT FAILED (rc=$PREFLIGHT_RC) -- not opening a window" >&2
  cat "$WT/evals/tier2/t3_64col_admission_preflight.log" >&2
  exit 2
fi
echo "[run_64col] preflight OK; all runnable arms ADMIT" >&2
...
exec env PYTHONPATH="$WT" "$PY" scripts/run_with_qwen_stopped.py \
  ...
  -- \
  bash "$WT/research/t3-64col/run_64col_inner.sh"
