...
A/B window measures its candidate against: the 19 family keys the benchmark
harness derives from its own flags plus the two FR-Spec keys its
`--full-frspec` block adds, **21 keys**. The registry declares the same 21.
