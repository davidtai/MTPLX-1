...
| Exact-BM8 linear-attention QKV subgroup | + retained GQA, NAX, and M8 K/V stack | 730.952 → 734.624 | 69.038 → 68.745 | 37.301 → 37.239 | **+0.1666%** | 35.34683 → 35.34683 | RETAINED by the wall gate; exact/deterministic with 3,168 new `m8_nax_k5120_n10240` routes per candidate arm. Decode TPS decreased while prefill drifted upward despite this route being decode-only, so the scheduled phase-stack rerun must revalidate this marginal win. | `post54-dflash2-m8-qkv-on-gqa678-m8o-m7o-m7z-m8kv-python16384in-1024out-t1-isolated-abba-2026-08-24.json` |
| Exact-BM8 MLP gate/up subgroup | + retained GQA, NAX, M8 K/V, and M8 linear-QKV stack | 738.670 → 743.761 | 69.045 → 69.482 | 37.055 → 36.803 | **+0.6850%** | 35.34683 → 35.34683 | RETAINED; exact/deterministic with 7,392 new `m8_nax_k5120_n17408` routes per candidate arm and every prior subgroup live | `post54-dflash2-m8-mlp-on-gqa678-m8o-m7o-m7z-m8kv-m8qkv-python16384in-1024out-t1-isolated-abba-2026-08-24.json` |
| Exact M=5 K-split specialization | + full retained M8 subgroup stack | 740.243 → 751.438 | 69.006 → 69.815 | 37.014 → 36.502 | **+1.4042%** | 35.34683 → 35.34683 | RETAINED; exact/deterministic with all 16,960 calls moved from padded-M6 to exact-M5, including 3,840 live `(5120,48)` calls | `post54-dflash2-m5-exact-on-full-m8-stack-python16384in-1024out-t1-isolated-abba-2026-08-24.json` |
| Shape-specific M=6 one-way K split | + exact-M5 and full retained M8 subgroup stack | 763.251 → 749.514 | 69.635 → 72.183 | 36.202 → 36.085 | **+0.3232%** | 35.34683 → 35.34894 | RETAINED as decode-only; `(5120,10240)` and `(5120,17408)` moved to one-way K split for 4,960 calls/arm, reducing deterministic candidate cycles 202→196. Cross-variant token drift is allowed at temperature 1. | `post54-dflash2-m6-kp1-selected-shapes-on-m5-full-m8-stack-python16384in-1024out-t1-isolated-abba-2026-08-24.json` |
...
| **Barrier-free M6 K=1 follow-up** | Final phase-corrected stack → direct single-part writeback | 712.698 → 721.119 | 68.081 → 68.304 | 38.124 → 37.765 | **+0.9521%** | 35.34894 → 35.34896 | **RETAINED FINAL FOLLOW-UP**; exact/deterministic, removes redundant threadgroup store/barrier/reload on both selected M6 K=1 shapes. All M5/M6 partition and KCONST alternatives plus M7/M8 NSG/scalar alternatives were rejected; full table is in `qwen38-challenge-performance-tables.md`. | `followup-m6-barrier-free-kp1-on-final-stack-python16384in-1024out-t1-isolated-abba-2026-08-24.json` |
...
| M=5 exact, selected M=6 split, padded-M7/BM8, exact-M8 subgroups | Decode-only | QuantizedLinear dispatcher excludes explicit prefill and routes only verify row counts 5--8. Final winner set keeps exact-M5 K=2; barrier-free M6 K=1 on QKV/MLP; padded-M7 output/linear-Z; and exact-M8 K/V, QKV, and MLP. Exact-M8 output is removed. M5/M6 KCONST and alternate partitions, exact-scalar M7, and NSG4/16 M7/M8 alternatives are exhausted and rejected. |
