...
| 11 | 63 | 5.7425% | `62174dbbca88` | `40a33f553244` | +230/-16 | REOPENED FOR ADAPTIVE MTP: reconcile the source cost-model schedule against MTPLX's existing streak, expected-value, and cost policies, then gate any missing live policy chronologically. Seed warming and row-8 early-flush portions may still be covered. |
...
| 48 | 543 | 0.1230% | `86fb1f020fc1` | `d2962993b6da` | +422/-240 | STAGED: the source's Q/K scalar memo is already present in MTPLX capture as host scalars, and its transient row-47 affine-2 QMV removal is irrelevant because that argmax selector is absent. The live target-side mechanism is adapted to the 64-layer BF16/5120 route: post-attention residual+RMSNorm is fused, and each interior layer carries `(base, delta)` so the prior exit add and next input RMSNorm become one launch. Explicit forward and merged-boundary counters prevent an inert route from passing. It awaits its chronological 16K cumulative gate. |
...
| 70 | 1063 | 3.9125% | `6f1cd66fc214` | `0dd6cffb1309` | +1070/-496 | STAGED IN PART: the clustered argmax selector and island-arm research controls are non-transferable, but the candidate-owned affine-4/group-64 wide QMV is live for MTP linears at M=3...9. The exact source arithmetic is ported: direct nibbles, source row partition, BF16 chunk sums, and a once-per-activation sum table consumed for M>=4; only exact Q4/group-64 BF16 contiguous cells with K divisible by 512 and N>=4096 route. It is scoped to the MTP block so the target's Q4/group-32 trunk is untouched. Kernel compilation/parity and its chronological 16K cumulative gate remain pending. |
