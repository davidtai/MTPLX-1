...
| 70 | 1063 | 3.9125% | `6f1cd66fc214` | `0dd6cffb1309` | +1070/-496 | STAGED IN PART: the clustered argmax selector and island-arm research controls are non-transferable, but the candidate-owned affine-4/group-64 wide QMV is live for MTP linears at M=3...9. The exact source arithmetic is ported: direct nibbles, source row partition, BF16 chunk sums, and a once-per-activation sum table consumed for M>=4; only exact Q4/group-64 BF16 cells with K divisible by 512 and N>=4096 route. Its first real-artifact conditioner exposed that MLX Python arrays have no public `.strides`; no timed arm ran. Delegating contiguity to `metal_kernel` then produced an implicit primitive outside the enclosing compiled graph, again before timing. The corrected boundary now inserts explicit `mx.contiguous` graph nodes for x/weight/scales/biases and disables the kernel's implicit copies. It remains scoped to the MTP block so the target's Q4/group-32 trunk is untouched; real Metal parity and the chronological 16K gate remain pending. |
