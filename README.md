...
MTPLX has an opt-in target-only AR path for two pinned community MoE Q4
artifacts that streams only routed experts into a bounded hot bank. It is not
enabled by default and has not earned a full-checkpoint performance claim. The
artifact restrictions, memory plans, manifest commands, parity gates, and
benchmark procedure live in the [advanced SSD-streamed MoE
guide](docs/advanced/ssd-streamed-moe.md).
