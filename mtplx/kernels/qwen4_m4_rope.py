...
WHY THIS EXISTS
---------------
A node census of the compiled fixed-M4 verify body found ``qsa_rope`` --
...
(``MTPLX_FABLE_OPDIET=1`` with the ``rope`` item) one QSA layer issues its
rotation as a chain of small elementwise and copy dispatches:
...
* **No indexer query prep.**  That rope site of the same QSA layer already
  has a shipped, pinned-bit-exact kernel
  (``qsa_indexer_prepare_queries_metal``); see
  ``MTPLX_FABLE_VERIFY_GLUE_ITEMS=qsa_rope_idx``.
...
expression and the verify-glue microbenchmark
...
#:
#: Derived from a census of the COMPILED body (``qsa_rope``, 288 matched
