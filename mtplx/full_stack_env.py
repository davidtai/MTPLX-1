...
The other half is the RETAINED FABLE SET: the twelve decode keys and the
eight prefill keys the PR-391 battery exported by hand from two files. Those
...
drift apart. Nothing else in the tree sets any of the twenty -- not the
...
:data:`FULL_STACK_KEYS` is the union: the 41 keys a fully armed serve must
...
readers. For those nine keys a profile stamp there arms NOTHING: the
...
* :data:`FULL_STACK_PROFILE_ENV`, the 23 keys the ``turbo-full-stack``
  ...
  server's auto-arm deliberately yields to one). All 23 ARE in that set, so
  ...
  back;
* :data:`FULL_STACK_RESTACK_ENV`, the full 41-key block, kept as the
...
* :func:`warn_unknown_family_keys`, which says at startup that an
  ...
  (``_HOST_TRIMS``, ``_K20_LOG``, ``_DEVICE_K20``, ``_QSA_M4_KT``,
  ``_COMPILED_DRAFT``, ...) that this registry has no reason to enumerate,
...
class EnvKeySpec:
    ...
    #: with neither a W84 lane nor an entry here is an unprovable arm --
...
REGISTERED_KEYS: tuple[EnvKeySpec, ...] = (
    ...
    # any of them, which is why the W49 battery had to export the file.
    EnvKeySpec(
    ...
    EnvKeySpec(
        ...
        note=(
            ...
            "MTPLX_FABLE_QSA_SPARSE_DRAFT (the M=1 twin) is deliberately NOT "
            "in the stack: the retained dispatch census shows zero QSA "
            "attention dispatches at M=1."
        ),
    ),
    EnvKeySpec(
    ...
    EnvKeySpec(
        ...
        note=(
            ...
            "cannot change a single attention call, and the W84 receipt "
    ...
    EnvKeySpec(
        ...
        note=(
            ...
            "NOTE scripts/fable/server_cell_bench.py's COMMON_ENV exports "
            "MTPLX_SESSION_BANK_MAX_BYTES=auto, so a run through that harness "
            "gets the auto-sizer rather than this value -- an export wins, "
            "which is the mechanism working, not a conflict."
        ),
    ),
...
#: Just the ABBA control arm -- the 21 keys ``tests/test_full_stack_profile``
#: re-derives from ``scripts/fable/abba_driver.py`` itself. Kept separate
#: from :data:`FULL_STACK_KEYS` because that derivation is a two-sided check
#: against the driver, and the Fable sets have a different second side (the
#: committed flag files, see :data:`GROUP_FLAG_FILES`).
...
#: The ABBA control arm alone, i.e. exactly one
#: ``scripts/fable/abba_driver.py`` invocation carrying
...
# ---------------------------------------------------------------------------
#
# The retained stack stopped being an opt-in on 2026-09-03. The 23 keys above
...
def fable_default_env(
    ...
    *,
    ...
    pre-import stamp asks for only its nine.
    """
