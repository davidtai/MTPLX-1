...
def _verify_sdpa(q, k, v, *, scale, mask):
    ...
    # Outside the verify-band head-chunk regime, fall through to the QSA dense
    # prefill lane (query-tiling / mask-fuse), which itself degrades to a plain
    # fused SDPA when its own flags are off. This composes PR #482 (verify
    # head-chunk) with PR #475 (aux-lanes QSA dense) at the single shared call
    # site: both features keep their opt-in flags and their off-by-default
    # behaviour (both unarmed == one fused scaled_dot_product_attention).
    return _qsa_dense_attention(q, k, v, mask=mask, scale=scale)
