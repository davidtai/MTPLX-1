"""Install-time verdicts for the armed flags that had none.
...
the benchmark driver, ``mtplx/prefill_bench.py``) passes through.
Each lane prints at most once per process; ``emit`` is idempotent.

RESTACK-PROFILE COMPATIBILITY
-----------------
...
the restack profile's the moment it exists.  Nothing here imports that module at module scope,
...
def print_install_receipt(
    ...
    never diverge, and otherwise runs that module's body verbatim.  Never raises: a
...
# ---------------------------------------------------------------------------
# 10. MTPLX_FABLE_ROUTE_KERNEL  (defaults-on needs a verdict per key)
# ---------------------------------------------------------------------------
...
#: Every env key that has a registered install verdict.  The sanity stage
