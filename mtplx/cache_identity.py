...
the benchmark driver on every A/B arm and by the server's
...
decline.  One decode lane measured that failure mode directly: a miss let the
layer run the stock recurrence from a state the dispatch had already replaced,
so every host-side counter stayed perfectly self-consistent while three ABBA
seeds produced different text, diverging around token ten, and nothing in the
receipts could see it.

This module is the generic repair, so a lane that reads a cache container by
identity cannot be hidden by the same re-wrap:
...
2. **A declared expectation.**  A dispatch that has already committed to a
   layer consuming something -- a substituted state leaf in slot 1 is the
   archetype -- registers ``expect(lane, layer_index)``.  The lane
...
3. **No ragged metadata across the re-wrap**.  The throwaway carries
...
def note_resolved_index(lane: str, layer_index: int) -> None:
    ...
    For lanes that already know the layer index rather than looking one up
    from their own scope.  A no-op outside an expectations scope.
    """
...
#: (``_mtplx_verify_rows``, ``_mtplx_verify_ple``, ...).
...
def assert_no_ragged_metadata(entry: Any, layer_index: int, *, label: str) -> None:
    ...
    * ``_fused_step_applies`` / ``_fused_conv_norm_applies`` /
    ...
    the eager path would have run on the same cache -- the failure mode this module exists for.
