...
@lru_cache(maxsize=None)
def _m6_grouped_quad_qmv_kernel(
    size_k: int,
    size_n: int,
    routed_input: bool,
):
    """Build exact-M6 QMV with one staged weight tile per repeated expert."""

    if (size_k, size_n, routed_input) not in (
        (4096, 2048, False),
        (2048, 4096, True),
    ):
        raise ValueError("Mia grouped quad QMV requires an exact gate/up or down bank")
    descriptor_plan = _mcg_quad_descriptor_plan()
    descriptors = ",".join(str(value) for value in descriptor_plan.descriptors)
    header = f"""
        using namespace metal;
        ...
        constant constexpr uint TOPK = 6;
        constant constexpr uint HAD = 128;
        constant constexpr uint MAX_ROUTES = 6;
        constant constexpr uint TILE_WORDS = 48;
        constant constexpr uint TILE_VECTORS = 6;
        constant constexpr uint BLOCK_TILES = 8;
        constant constexpr uint BLOCK_TILES_N = 16;
    ...
    for route in range(6):
        x_row = f"task{route}" if routed_input else f"task{route} / TOPK"
        route_setup.append(
            f"""
            uint task{route} = 0u;
            size_t x_row{route} = 0u;
            if (active_routes > {route}u) {{
                task{route} = uint(packed_tasks[first_route + {route}u]);
                x_row{route} = {x_row};
            }}
            """
        )
        route_transforms.append(
            f"""
            if (active_routes > {route}u) {{
                half scaled{route} = half(
                    x[x_row{route} * SIZE_K + k]
                    * suh[(size_t)expert * SIZE_K + k]
                );
                float transformed{route} = hadamard_h128(
                    float(scaled{route}), lane, had_values
                );
                x_had[{route}u * HAD + lane] = half(
                    transformed{route} * HAD_SCALE
                );
            }}
            """
        )
        accumulator_declarations.append(
            f"float accumulator0_{route} = 0.0f; float accumulator1_{route} = 0.0f;"
        )
        route_outputs.append(
            f"""
            if (active_routes > {route}u) {{
                float output_had0_{route} = hadamard_h128(
                    float(half(accumulator0_{route})), lane, had_values
                );
                half rotated0_{route} = half(output_had0_{route} * HAD_SCALE);
                y[(size_t)task{route} * SIZE_N + n0] = half(
                    rotated0_{route}
                    * svh[(size_t)expert * SIZE_N + n0]
                );
                float output_had1_{route} = hadamard_h128(
                    float(half(accumulator1_{route})), lane, had_values
                );
                half rotated1_{route} = half(output_had1_{route} * HAD_SCALE);
                y[(size_t)task{route} * SIZE_N + n1] = half(
                    rotated1_{route}
                    * svh[(size_t)expert * SIZE_N + n1]
                );
            }}
            """
        )
    ...
    for quad_row, decoder in enumerate(
        ...
        route_accumulates = []
        for route in range(6):
            route_accumulates.append(
                f"""
                if (active_routes > {route}u) {{
                    float value{quad_row}_{route}_0 = float(
                        x_had[{route}u * HAD + local_k{quad_row}]
                    );
                    accumulator0_{route} += value{quad_row}_{route}_0
                        * float(weights{quad_row}_0.x);
                    accumulator1_{route} += value{quad_row}_{route}_0
                        * float(weights{quad_row}_1.x);
                    float value{quad_row}_{route}_1 = float(
                        x_had[{route}u * HAD + local_k{quad_row} + 1u]
                    );
                    accumulator0_{route} += value{quad_row}_{route}_1
                        * float(weights{quad_row}_0.y);
                    accumulator1_{route} += value{quad_row}_{route}_1
                        * float(weights{quad_row}_1.y);
                    float value{quad_row}_{route}_2 = float(
                        x_had[{route}u * HAD + local_k{quad_row} + 2u]
                    );
                    accumulator0_{route} += value{quad_row}_{route}_2
                        * float(weights{quad_row}_0.z);
                    accumulator1_{route} += value{quad_row}_{route}_2
                        * float(weights{quad_row}_1.z);
                    float value{quad_row}_{route}_3 = float(
                        x_had[{route}u * HAD + local_k{quad_row} + 3u]
                    );
                    accumulator0_{route} += value{quad_row}_{route}_3
                        * float(weights{quad_row}_0.w);
                    accumulator1_{route} += value{quad_row}_{route}_3
                        * float(weights{quad_row}_1.w);
                }}
                """
            )
        quad_blocks.append(
    ...
    source = f"""
        ...
        uint expert = uint(block_expert[packed_block]);
        uint first_route = uint(block_row[packed_block]);
        uint active_routes = uint(block_size[packed_block]);
        uint n0 = n_block * 256u + lane;
        uint n1 = n0 + HAD;
        {''.join(route_setup)}

        threadgroup float had_values[HAD];
        threadgroup half x_had[MAX_ROUTES * HAD];
        threadgroup uint4 packed_tile_vectors[
            BLOCK_TILES * STAGE_VECTORS_PER_K_TILE
        ];
        threadgroup ushort* packed_tiles =
        ...
        {' '.join(accumulator_declarations)}
        for (uint k_block = 0; k_block < KBLOCKS; ++k_block) {{
            uint k = k_block * HAD + lane;
            {''.join(route_transforms)}
    ...
    return mx.fast.metal_kernel(
        name=(
            f"mtplx_dsv4_exl3_m6_grouped_quad_mcg_qmv_k{size_k}_n{size_n}"
            f"_e216_t6_r{int(routed_input)}_bn256_bm6_u4stage_v1"
        ),
        input_names=[
            "x",
            "trellis",
...
class _InstalledM6GroupedQMVPlan(NamedTuple):
    geometry: tuple[int, int, int, int, float, int, int, int]
    route_pack: Any
    hidden_to_intermediate: Any
    intermediate_to_hidden: Any
...
class EXL3SwitchGLU(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self._m6_grouped_qmv_plan = None
    ...
    def install_m6_grouped_qmv_runtime(self) -> None:
        """Bind repeated-expert staging without changing direct-M6 arithmetic."""

        if self._m6_quad_qmv_plan is None:
            raise ValueError("Mia grouped QMV requires the exact quad plan")
        geometry = (
            self.hidden_size,
            self.gate_proj.output_dims,
            self.experts,
        ...
        if geometry != (4096, 2048, 216, 6, 10.0, 256, 36, 6):
            raise ValueError(f"Mia grouped QMV geometry changed: {geometry!r}")
        self._m6_grouped_qmv_plan = _InstalledM6GroupedQMVPlan(
            geometry=geometry,
            route_pack=_trellis_route_pack_kernel(216, 6, 8),
            hidden_to_intermediate=_m6_grouped_quad_qmv_kernel(
                4096, 2048, False
            ),
            intermediate_to_hidden=_m6_grouped_quad_qmv_kernel(
                2048, 4096, True
            ),
        )
    ...
    def direct_qmv_m6_grouped(
        self,
        x: mx.array,
        expert_ids: mx.array,
    ) -> mx.array:
        """Run exact-M6 arithmetic while staging each selected expert once."""

        original_dtype = x.dtype
        x_half = x.astype(mx.float16)
        plan = self._m6_grouped_qmv_plan
        (
            packed_tasks,
            _inverse,
            _sorted_ids,
        ...
        gate = self._m6_grouped_project(
            self.gate_proj,
            x_half,
            route_blocks,
            plan.hidden_to_intermediate,
        )
        up = self._m6_grouped_project(
            self.up_proj,
            x_half,
            route_blocks,
            plan.hidden_to_intermediate,
        ...
        up = mx.clip(up, -self.limit, self.limit)
        activated = (nn.silu(gate) * up).astype(mx.float16)
        return self._m6_grouped_project(
            self.down_proj,
            activated.reshape(36, self.down_proj.input_dims),
            route_blocks,
            plan.intermediate_to_hidden,
