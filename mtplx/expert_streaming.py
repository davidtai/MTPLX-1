...
class LayerExpertSlotBank:
    ...
    def commit_resolved_all_hits(
        ...
        *,
        ...
        """Validate device-resolved hits while preserving normal accounting."""
        ...
        try:
            slots = tuple(
                _integer("slot", slot, minimum=0) for slot in resolved_slots
            )
        except TypeError as exc:
        ...
        if any(
            self._expert_to_slot.get(expert) != slot
            for expert, slot in zip(experts, slots, strict=True)
        ):
            return None
