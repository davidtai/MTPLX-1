...
DFLASH2_TARGET_REPO = "Youssofal/Qwen3.8-27B-MTPLX-Optimized-Speed"
DFLASH2_TARGET_BASE_REPO = "Qwen/Qwen3.8-27B"
DFLASH2_TARGET_REVISION = "123db8bcc7101455b00d9aad36c0e760c6e7de02"
DFLASH2_TARGET_CONFIG_SHA256 = "533e833dedb9e7b6a8ee22ab4f2fc034bcf6ded9d8693e5ebcc9d5f159b62a3b"
DFLASH2_TARGET_WEIGHT_SHA256 = {
    "model-00001-of-00004.safetensors": "aed435f4011667af7772fee7ccec90c86f5a6c85649ef74fe6e397a7504b4578",
    "model-00002-of-00004.safetensors": "3f1960306e36255b7f0c7e80c742f03d96131561b766b1f6af9ca138ff77f939",
    "model-00003-of-00004.safetensors": "6525c0edae616c0f62b69fc190268c69bdf7a18b0b2ad3de007f3e4464387471",
    "model-00004-of-00004.safetensors": "cef192620e5ecb23eaac19d2c041edc44d9cbb9fb10265bf1213b649c3eae0d9",
    "model-vision.safetensors": "964bef26c740bdb6fe464b4c7c48840d4c952ea597c181c41cb131c65ea3c5d5",
    "mtp.safetensors": "4468f39621de68a19ffd0bcb2e2e2f352205def7436a625b3427e3752866c287",
}
DFLASH2_DRAFT_REPO = "z-lab/Qwen3.8-27B-DFlash2"
DFLASH2_DRAFT_REVISION = "50307d4c4cde6860d4eee73e2547cd786fe8e8a4"
DFLASH2_DRAFT_CONFIG_SHA256 = "873e3556509b0da06e29654ba00d4944888d4b5e8a33afde25f7eb27d321e980"
DFLASH2_DRAFT_WEIGHT_SHA256 = {
    "model.safetensors": "67fc76d68dc5a9415511a4f394ef744d67510cd20e93b37cc2cc7d28e4bab65c",
}
DFLASH2_ALGORITHM_REPO = "davidtai/dflash-mlx"
DFLASH2_ALGORITHM_REVISION = "c5b76ddb62bdefb6eeef1282641842edcf23a1b8"
DFLASH2_DEFAULT_SAMPLER = {"temperature": 1.0, "top_p": 0.95, "top_k": 20}
DFLASH2_DRAFT_LAYERS = 5
DFLASH2_DEFAULT_BLOCK_SIZE = 8
...
def resolve_dflash2_bundle_paths(bundle_root: str | Path) -> dict[str, Any] | None:
    ...
    raw_block_size = metadata.get(
        "block_size", draft_section.get("block_size", DFLASH2_DEFAULT_BLOCK_SIZE)
    )
    try:
        draft_block_size = int(raw_block_size)
    except (TypeError, ValueError) as exc:
        raise ValueError(f"{DFLASH2_BACKEND} block_size must be an integer") from exc
    if not 1 <= draft_block_size <= 8:
        raise ValueError(f"{DFLASH2_BACKEND} block_size must be in the physical range 1..8")
    return {
...
def _checksum_entries(metadata: dict[str, Any], name: str) -> list[tuple[str | None, str | None]]:
    ...
    aliases = {
        ...
        "target_weights": ("target_weights", "target/weights"),
    ...
    for value in values:
        if isinstance(value, str):
            ...
            elif name.endswith("_weights") and "files" in value and isinstance(value["files"], list):
...
def _validate_checksums(metadata: dict[str, Any], root: Path, target_dir: Path | None, draft_dir: Path | None) -> list[str]:
    ...
    declared_weight_paths: dict[str, set[str]] = {
        "target_weights": set(),
        "draft_weights": set(),
    }
    for name, default in defaults.items():
        ...
        if not entries:
            ...
            continue
        weight_dir = (
            target_dir if name == "target_weights"
            else draft_dir if name == "draft_weights"
            else None
        )
        weight_files = _weight_files(weight_dir)
        if name.endswith("_weights") and len(entries) == 1 and entries[0][0] is None:
        ...
        for relative, digest in entries:
            ...
            if observed != digest.casefold():
                errors.append(
                    f"{name} checksum mismatch for {relative}: "
                    f"expected {digest.casefold()}, observed {observed}"
                )
            if name.endswith("_weights"):
                declared_weight_paths[name].add(str(candidate.relative_to(root)))
    for name, weight_dir in (
        ("target_weights", target_dir),
        ("draft_weights", draft_dir),
    ):
        if weight_dir is None:
            continue
        observed_weight_paths = {
            str(path.relative_to(root)) for path in _weight_files(weight_dir)
        }
        missing = observed_weight_paths - declared_weight_paths[name]
        extra = declared_weight_paths[name] - observed_weight_paths
        if missing:
            errors.append(
                f"{name.replace('_', ' ')} checksum omits files: "
                + ", ".join(sorted(missing))
            )
        if extra:
            errors.append(
                f"{name.replace('_', ' ')} checksum declares non-weight files: "
                + ", ".join(sorted(extra))
            )
    return errors


def _validate_trusted_artifact_identity(
    target_dir: Path | None, draft_dir: Path | None
) -> list[str]:
    """Compare actual artifact bytes with the independently measured trust root."""

    errors: list[str] = []
    for label, model_dir, config_digest, weight_digests in (
        (
            "target",
            target_dir,
        ...
        if config_path.is_file():
            ...
            if observed_config != config_digest:
                errors.append(
                    f"trusted {label} config digest mismatch: expected "
                    f"{config_digest}, observed {observed_config}"
                )
        observed_files = {
            str(path.relative_to(model_dir)): path for path in _weight_files(model_dir)
        }
        if set(observed_files) != set(weight_digests):
            errors.append(
                f"trusted {label} weight inventory mismatch: expected "
                f"{', '.join(sorted(weight_digests))}; observed "
                f"{', '.join(sorted(observed_files))}"
            )
        for name in sorted(set(observed_files) & set(weight_digests)):
            observed_digest = _sha256_file(observed_files[name])
            expected_digest = weight_digests[name]
            if observed_digest != expected_digest:
                errors.append(
                    f"trusted {label} weight digest mismatch for {name}: expected "
...
def validate_dflash2_manifest(metadata: dict[str, Any], bundle_root: str | Path | None = None) -> list[str]:
    """Return actionable manifest and byte-identity errors."""
    ...
    expected_identity = {
        "target.repo": (_metadata_repo(metadata, "target"), DFLASH2_TARGET_REPO),
        "target.base_model": (_metadata_base_model(metadata), DFLASH2_TARGET_BASE_REPO),
        "target.revision": (_metadata_revision(metadata, "target"), DFLASH2_TARGET_REVISION),
        "draft.repo": (_metadata_repo(metadata, "draft"), DFLASH2_DRAFT_REPO),
        "draft.revision": (_metadata_revision(metadata, "draft"), DFLASH2_DRAFT_REVISION),
        "algorithm.repo": (_metadata_repo(metadata, "algorithm"), DFLASH2_ALGORITHM_REPO),
        "algorithm.revision": (
            _metadata_revision(metadata, "algorithm"), DFLASH2_ALGORITHM_REVISION
        ),
    }
    for label, (observed, expected) in expected_identity.items():
        if str(observed or "").casefold() != expected.casefold():
            errors.append(f"{label} must equal the measured identity {expected}")
    ...
    if precision not in DFLASH2_DRAFT_PRECISIONS:
        ...
        errors.append("draft precision must equal the measured 4bit contract")
    if bundle_root is not None:
        ...
        errors.extend(_validate_trusted_artifact_identity(target_dir, draft_dir))
    return errors
...
def _validate_bundle(paths: dict[str, Any] | None, bundle_root: Path | None = None) -> tuple[list[str], dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        validate_qwen38_27b_contract(target_config, Path(DFLASH2_TARGET_REPO))
    except Exception as exc:
        errors.append(f"target is not the exact Optimized-Speed contract: {exc}")
    ...
    qwen_target = "qwen3" in target_text and (
        ...
        or (target_base_model or "").casefold() == DFLASH2_TARGET_BASE_REPO.casefold()
    )
    if not qwen_target:
    ...
    elif target_hidden != 5120:
        errors.append("Qwen3.8 Optimized-Speed target hidden_size must equal 5120")
    if target_vocab is None:
        errors.append("target config must declare a positive vocab_size")
    elif target_vocab != 248320:
        errors.append("Qwen3.8 Optimized-Speed target vocab_size must equal 248320")
    ...
    if not isinstance(layer_ids, list) or not layer_ids:
        ...
        elif tuple(layer_ids) != DFLASH2_TARGET_LAYER_IDS:
            errors.append(
                "target_layer_ids must equal the measured geometry "
                f"{DFLASH2_TARGET_LAYER_IDS}"
            )
    block_size = paths.get("draft_block_size") if isinstance(paths, dict) else None
    if block_size != DFLASH2_DEFAULT_BLOCK_SIZE:
        errors.append("DFlash2 physical block size must equal the measured width 8")
