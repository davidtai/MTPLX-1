"""Overlap the fixed-M4 verify graph's host construction with GPU work.

What this lane targets, measured on the RETAINED stack
------------------------------------------------------
The decode-cycle census found, on the PR391 device-D3 arm, that
...
* 0.496 ms/cycle -- the PLE auxiliary's own submission gap (gap A),
...
The N-layer prefix
-----------------------
The first partition stopped at layer 0 because layer 0 is the only layer that reads no PLE
auxiliary.  ``MTPLX_FABLE_GRAPH_BUILD_OVERLAP_LAYERS=N`` (default 1, i.e.
the layer-0 partition) moves the seam to layer ``N-1 / N``.  Saving is
...
where the layer-0 partition prepared it, so the default arm is that schedule.
...
#: how many leading target layers the prefix graph carries.  ``1`` is
#: the layer-0 prefix, i.e. the default is exactly the shipped behaviour.
...
_RECEIPT_ZERO: dict[str, float] = {
    ...
    # the prefix depth actually INSTALLED (not the one requested).  Set
    ...
    "prefix_layers": 0,
    # windows whose PLE auxiliary was built at the ENQUEUE (the hoist),
