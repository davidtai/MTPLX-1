...
from .runtime_options import fable_qsa_sparse_decode_enabled
...
class FixedM4OverlapPrefix:
    """One layer-0 result queued ahead of this window's D3 host sync.
    ...
    generation: int
    committed_count: int
    #: ``None`` at depth 1, where the prefix reads no PLE auxiliary and
...
class TensorOffsetQSACache:
    ...
    def __init__(
        ...
        *,
        ...
        # MTPLX_FABLE_QSA_SPARSE_DECODE: the native split-K
        ...
        # an armed flag that reaches a cache constructed WITHOUT it is
        ...
        if self.fable_qsa_sparse_decode:
            ...
            if not _qsa_sparse.install(
                ...
                raise RuntimeError(
                    "MTPLX_FABLE_QSA_SPARSE_DECODE is armed but the "
...
# the same sharing, for the graph-build-overlap pair.  Without it the
    ...
    *state_out)`` is a pure function: every piece of cache state enters as an
    ...
    def __init__(
        ...
        *,
        ...
        # MTPLX_FABLE_GRAPH_BUILD_OVERLAP (default off).  One slot, not a
        ...
        # the prefix depth this bank compiled its overlap pair at.  Set
        # by ``arm_fixed_m4_graph_build_overlap`` from
        # ``MTPLX_FABLE_GRAPH_BUILD_OVERLAP_LAYERS``; 1 is the layer-0 partition.
        self._fixed_m4_overlap_layers: int = (
            _graph_build_overlap.DEFAULT_LAYERS
        )
        # time the FIRST replay of each graph -- where mx.compile
    ...
    @staticmethod
    def _verify_state_in(plan):
        """``state_in`` for one state-plan slice, in plan order."""
    ...
    def _forward_installed_fixed_m4(
        ...
        *,
        ...
        ``compiled_aux`` is the overlap pair's only concession: when the graph-build
        ...
        state_in = self._verify_state_in(dispatch["state_plan"])
    ...
    # -- graph-build overlap (MTPLX_FABLE_GRAPH_BUILD_OVERLAP) -----
    #
    ...
    def _make_fixed_m4_overlap_prefix_step(
        ...
        *,
        ...
        """The traced closure for layers ``0..layer_count-1``.
        ...
        traces exactly the graph the layer-0 partition traced.
        """
    ...
    def _make_fixed_m4_overlap_suffix_step(
        ...
        *,
        ...
        """The traced closure for layers ``start_layer..last`` + head."""
    ...
    def install_fixed_m4_overlap_split(self, layer_count: int) -> None:
        """Compile the ``0..N-1`` / ``N..last`` pair for THIS lane.
    ...
    def enqueue_fixed_m4_overlap_prefix(
        ...
        **The hoist.**  At depth 1 the prefix reads no PLE auxiliary and
        ...
        if dispatch["donate"]:
            ...
            self._clear_shadow_leaf_refs()
        state_in = tuple(
            self._verify_state_in(dispatch["state_plan"][:layer_count])
        )
        compiled_aux = None
        if needs_aux:
        ...
        *,
        ...
        if (
            ...
            return self._forward_installed_fixed_m4(
                ...
                committed_count,
                cache,
                # a refused prefix may already have paid for this
        ...
        state_in = self._verify_state_in(suffix_plan)
    ...
    def demote(self, cache: Any) -> int:
        ...
        if count:
            ...
            # a queued layer-0 prefix was traced against the shadow that
    ...
    def _make_verify_step(
        ...
        *,
        ...
        def verify_step(input_ids, *args):
            ...
            # A trace-time engagement check, on the graph this body just built
            # and fatal, because the graph is what the next few hundred cycles
            # replay: an armed sparse-decode flag that never routed is an
            # inert arm, which costs a measurement rather than an answer.
