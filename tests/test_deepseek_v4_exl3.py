...
def test_authentic_mia_m6_grouped_direct_matches_quad_final_bits():
    """Repeated experts may share weight staging without changing M6 arithmetic."""
    ...
    grouped_plan_type = exl3._InstalledM6GroupedQMVPlan
    grouped_kernel = exl3._m6_grouped_quad_qmv_kernel
    switch = EXL3SwitchGLU(4096, 2048, 216, 6, limit=10.0)
    with safe_open(_LAYER0, framework="np") as handle:
        for bank, weight in (
            (switch.gate_proj, "w1"),
            ...
            prefix = f"layers.0.ffn.experts.0.{weight}.rank0"
            bank.trellis = mx.repeat(
                mx.array(handle.get_tensor(f"{prefix}.trellis"))[None],
                6,
                axis=0,
            )
            bank.suh = mx.repeat(
                mx.array(handle.get_tensor(f"{prefix}.suh"))[None],
                6,
                axis=0,
            )
            bank.svh = mx.repeat(
                mx.array(handle.get_tensor(f"{prefix}.svh"))[None],
                6,
                axis=0,
            )
    ...
    switch._m6_grouped_qmv_plan = grouped_plan_type(
        geometry=(4096, 2048, 216, 6, 10.0, 256, 36, 6),
        route_pack=exl3._trellis_route_pack_kernel(216, 6, 8),
        hidden_to_intermediate=grouped_kernel(4096, 2048, False),
        intermediate_to_hidden=grouped_kernel(2048, 4096, True),
    )
    x = mx.array(
        np.linspace(-1.0, 1.0, 6 * 4096, dtype=np.float32).reshape(6, 4096)
    ).astype(mx.bfloat16)
    ...
    direct = switch.direct_qmv_m6_quad(x, expert_ids)
    grouped = switch.direct_qmv_m6_grouped(x, expert_ids)
    mx.eval(direct, grouped)

    np.testing.assert_array_equal(
        np.array(direct.view(mx.uint16)),
        np.array(grouped.view(mx.uint16)),
    )
