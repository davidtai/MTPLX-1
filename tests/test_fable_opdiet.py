...
def _select_bank_update(pooled, candidate, safe_block, condition):
    ...
    Kept as a third value-identical spelling so the choice stays visible: it
    measured -20% against stock where the shipped rowsel form reached -49%,
    because both of its operands broadcast and MLX must emit a general
    (strided) select over the whole bank.
    """
