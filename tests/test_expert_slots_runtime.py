...
def test_lazy_pool_allocates_no_persistent_buffers_at_open(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "slabs" not in pool.snapshot()
        assert not hasattr(pool, "prepare_slab_reclaim")
        assert not hasattr(pool, "commit_slab_reclaim")
        assert not hasattr(pool, "abort_slab_reclaim")
        assert not hasattr(pool, "regrow_slab")
...
def test_direct_buffer_replacement_reuses_allocation_and_release_is_exact(
    ...
    try:
        ...
        assert pool.protected_slot_ids() == (0,)
        second.release(synchronize=False)
        assert pool.protected_slot_ids() == ()
...
def test_health_telemetry_reports_reader_integrity_errors(tmp_path: Path) -> None:
    pool, _allocator, _spec = _lazy_global_direct_pool(tmp_path)
    try:
