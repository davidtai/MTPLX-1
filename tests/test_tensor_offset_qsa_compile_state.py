...
def _tensor_offset_qsa_cache_type():
    ...
    # The class body reads one module-level name on its DEFAULT path (the
    # sparse-decode lane's armed-but-inert guard). Seed it with the real
    # function rather than a stub: ``mtplx.runtime_options`` is pure env
    # parsing and imports no MLX, so this test still never initializes the
    # framework.
