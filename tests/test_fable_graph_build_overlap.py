"""Gates for MTPLX_FABLE_GRAPH_BUILD_OVERLAP[_LAYERS].
...
#: The depths the N-layer prefix prices.  4 is the first that puts a QSA layer in the prefix
#: (layers 3, 7, ... are full attention), 2 the first that puts the PLE layer
#: there.  1 is the layer-0 partition and the flag's default.
DEPTHS = (1, 2, 3, 4)
...
def test_prefix_is_rooted_immediately_and_never_read_on_the_host(
    ...
    if PLE_INDEX < depth:
        ...
        # depth 1 reads no auxiliary: exactly one submission, the layer-0 one.
...
def test_the_hook_passes_the_same_array_the_monolithic_route_would():
    ...
    # The aux hoist rides the SAME three arguments the verify site passes, so
...
def test_pr391s_layer0_split_is_left_alone():
    """The N-layer prefix adds a pair; it does not edit the device-committed lane's."""
    ...
    # the pair exists and is range-parameterized
...
# --------------------------------------------------------------------------
# Follow-up: construction cost, where it happens, and process-wide reuse
# --------------------------------------------------------------------------
...
def test_construction_happens_after_the_prefill_span_not_inside_it():
    """Follow-up gate: nothing this lane builds is inside prompt_eval.
