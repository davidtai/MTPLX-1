"""W77: a compiled GDN run must not silently drop ragged-batch metadata.
...
W66d's failure mode, one field over.
...
def test_the_assertion_names_the_layer_and_the_field():
    ...
    # A sibling of the W73 error, so an existing handler still catches it.
...
def _drive(cache, layers=None, monkeypatch=None):
    ...
    if monkeypatch is not None:
        # The fold is off; only the W77 guard is under test here.
