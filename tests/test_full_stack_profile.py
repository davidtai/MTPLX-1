...
#: The retained-stack CONTROL ARM: the benchmark harness's own control flags
#: plus its --full-frspec block, 21 keys.
...
def test_the_profile_stamps_every_key_nothing_else_sets() -> None:
    """The gap plus both retained Fable groups -- 26 keys, and no others.
    ...
    nobody serving actually got. The three MTPLX_QWEN4_M4_ROUTED_* keys rode
    in the benchmark harness's own base environment rather than in either flag
    file, and so were missing from the served stack in the same way.
    """
    ...
    assert len(FULL_STACK_PROFILE_ENV) == 26
...
def test_a_plain_turbo_serve_is_short_the_whole_retained_stack() -> None:
    ...
    assert "20/44 driver-stack keys armed" in stack_summary_line(environ)
...
def test_the_profile_plus_the_server_auto_arm_completes_the_stack() -> None:
    ...
    assert "44/44 driver-stack keys armed" in line
    assert "(mtp, qwen4_exp fixed-M4 pack)" in line
    assert "profile 26" in line
...
def test_a_predicate_that_did_not_hold_reads_as_unarmed() -> None:
    ...
    assert "27/44 driver-stack keys armed" in line
