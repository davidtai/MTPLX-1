"""Every armed flag prints an install-time verdict, exactly once.
...
KEY_LANES = {
    ...
    # 2026-09-03: the retained stack is armed by DEFAULT for a
    # Flash-Next serve, and the program's rule is that every armed flag
    # prints an install-time verdict. These three had none. (Before that change this
...
def test_every_served_key_has_a_verdict():
    ...
    This is the guard the sanity stage exists to make permanent: a key
...
def test_armed_process_states_armed_or_resolved_for_every_key():
    ...
    assert states == {
        ...
        # the three lanes that had no verdict before the retained stack
