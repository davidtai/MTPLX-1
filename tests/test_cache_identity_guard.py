"""W73: a compiled run's container re-wrap must not be able to hide a lane.

``Qwen4ExpTextModel._compiled_run_fn`` hands each GDN layer a THROWAWAY
``ArraysCache(size=2)``, not the container its caller owns.  W66d measured
what that costs: the keep-mask fold's ``id(cache entry)`` lookup missed all 35
foldable layers, the dispatch had already substituted the ring's base into
state slot 1, and the recurrence silently ran from a state missing committed
windows -- for hours, with every host-side counter reading perfect.

The fold was repaired lane-specifically.  These tests pin the GENERIC guard,
driven through the REAL ``_compiled_run_fn``:
...
3. the keep-mask fold still resolves, through both its own alias and the
   generic one.
...
#: A synthetic lane's per-layer state, keyed the way W66d's fold was: by the
#: identity of the cache container the layer is handed.
...
def _drive_compiled_run(monkeypatch, *, resolve: bool, declare: bool = True):
    ...
    container it was handed (the W66d defect).  Returns
    ``(real, seen, run, flat)``.
    """
    ...
    # The fold is off; only the generic guard is under test here.
    monkeypatch.setattr(qm, "_GDN_KEEPMASK_FOLD_ARMED", False)
    ...
    if declare:
        ...
        # lane's rows -- exactly the fold's position.
...
def test_a_lane_hidden_by_the_rewrap_raises_at_trace_time(monkeypatch):
    """The W66d failure mode, generically: a miss, not a decline."""
...
# --------------------------------------------------------------------------
# Regression: the keep-mask fold still resolves
# --------------------------------------------------------------------------
...
def _drive_fold_through_compiled_run(monkeypatch, *, armed: bool, rewrap: bool):
    """The fold's own lane, through ``_compiled_run_fn`` under a rewrap scope."""
    ...
    shadow = [_Entry(f"shadow{index}") for index in range(3)]
    scope = fold.make_prefix_scope((0, 2), _trailing(2), shadow.__getitem__)
    flat = [mx.zeros((1, 2)) for _ in range(6)]
    error = None
    with fold.fold_prefix_scope(scope):
        with ci.rewrap_scope(shadow if rewrap else None):
            out = run(mx.zeros((1, 2)), *flat)
            mx.eval(out)
        try:
            ci.assert_satisfied((0, 1, 2), label="unit compiled run")
            ...
            error = failure
    return scope, seen, error


def test_fold_still_resolves_through_its_own_alias(monkeypatch):
    scope, seen, error = _drive_fold_through_compiled_run(
        monkeypatch, armed=True, rewrap=True
    )
...
def test_fold_resolves_through_the_generic_alias_alone(monkeypatch):
    """Belt and braces: the fold resolves even without ``bind_fold_alias``."""

    scope, seen, error = _drive_fold_through_compiled_run(
        monkeypatch, armed=False, rewrap=True
    )
    assert seen[0] == scope.by_layer[0]
    assert seen[2] == scope.by_layer[2]
    assert seen[1] is None
...
def test_fold_with_neither_alias_is_named_by_the_generic_guard(monkeypatch):
    """W66d, caught generically -- with the lane name and the layer index."""

    scope, seen, error = _drive_fold_through_compiled_run(
        monkeypatch, armed=False, rewrap=False
    )
    assert seen == {0: None, 1: None, 2: None}
    assert scope.consumed == 0
    # No rewrap scope means no alias was stamped, so the generic guard has
    # nothing to check -- the fold's own must-have-happened check is what
    # fires.  (In production `_decode_layers_compiled` always publishes the
    # source, which is what makes the generic guard fire first.)
    assert error is None
    with pytest.raises(
...
def test_generic_guard_names_the_fold_when_a_rewrap_hides_it(monkeypatch):
    """The production shape: the source IS published, the lane misses anyway."""
    ...
    class _Layer:
        ...
        def __call__(self, h, *, input_ids, ssm_mask, cache):
            # A lane that looks the container up raw -- W66d exactly.
            fold._PREFIX_SCOPE.get().by_entry.get(id(cache))
            cache[0] = cache[0] + 1
            cache[1] = cache[1] + 1
            return h + 1
    ...
    flat = [mx.zeros((1, 2)) for _ in range(6)]
    with fold.fold_prefix_scope(scope):
        with ci.rewrap_scope(shadow):
            mx.eval(run(mx.zeros((1, 2)), *flat))
        with pytest.raises(ci.CacheIdentityContractError) as excinfo:
            ci.assert_satisfied((0, 1, 2), label="unit compiled run")
    message = str(excinfo.value)
    assert fold.CACHE_IDENTITY_LANE in message
    assert "layer 0" in message and "layer 2" in message
