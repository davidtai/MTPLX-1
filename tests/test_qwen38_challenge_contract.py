...
def test_early_projection_candidates_compose_in_chronological_order(
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda model, *, active: calls.append(("packed_qkv", active))
        or {"configured_modules": 64, "active_modules": 64},
        raising=False,
    )
    monkeypatch.setattr(
        "mtplx.qwen38_challenge.configure_qwen3_next_gdn_projection_pairs",
        lambda model, *, active: calls.append(("gdn_projection_pairs", active))
        or {"configured_modules": 48, "active_modules": 48},
        raising=False,
    )
    runtime = MTPLXRuntime(
        model=SimpleNamespace(mtp_update_cache=_callable),
    ...
    route = install_qwen38_route(
        ...
        cache_route="control",
        packed_qkv=True,
        gdn_projection_pairs=True,
    )

    assert route.route_id == "packed_qkv+gdn_projection_pairs"
    assert route.kernel_ids == (
        "qwen38_attention_packed_qkv_s_le16_v1",
        "qwen38_gdn_projection_pairs_v1",
    )
    assert calls == [("packed_qkv", True), ("gdn_projection_pairs", True)]
    assert runtime.qwen38_feature_receipt == {
        "packed_qkv": {"configured_modules": 64, "active_modules": 64},
